...
class InternVLChatModel(PreTrainedModel):
    ...
    def forward(
        ...
        try:
            # print(f"[SHAPE_DEBUG] input_embeds[selected1].shape: {input_embeds[selected1].shape}")
            # print(f"[SHAPE_DEBUG] vit_embeds.reshape(-1, C).shape: {vit_embeds.reshape(-1, C).shape}")
            input_embeds[selected1] = input_embeds[selected1] * 0.0 + vit_embeds.reshape(-1, C)
            # print(f"[SHAPE_DEBUG] input_embeds[selected2].shape: {input_embeds[selected2].shape}")
            # print(f"[SHAPE_DEBUG] fast_feature.reshape(-1, C).shape: {fast_feature.reshape(-1, C).shape}")

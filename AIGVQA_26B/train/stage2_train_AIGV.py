...
class CustomTrainer(Trainer):
        ...
        with torch.no_grad():
            for item2 in eval_dataloader:
                ...
                output = self.model(
                    ...
                    # pixel_values3 = item["pixel_values3"][0].to(torch.bfloat16),
                ...
                data_list.append([video_name, score1, level])

        # Save the results and compute metrics
        # self.save_and_evaluate(data_list)
        self.save_and_evaluate(data_list, output_file=self.output_file, metrics_file=self.metrics_file)
    ...
    def save_and_evaluate(self, data_list, output_file='results.csv', metrics_file='metrics.txt'):
        ...
        df = pd.DataFrame(data_list, columns=['video_name', 'Overall_MOS','level'])
        # Save to CSV
        output_filer = output_file.split('.csv')[0]+f'_epoch{EPOCH}.csv'
        df.to_csv(output_filer, index=False)
        print(f'Results saved to {output_filer}')
